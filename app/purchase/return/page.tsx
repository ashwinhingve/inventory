...
export default function PurchaseReturnsPage() {
  ...
  // Count total quantity for a return
  const calculateTotalQuantity = (returnItem: PurchaseReturn) => {
    return returnItem.items.reduce((total, item) => total + item.quantity, 0);
  };
  ...
  const handleDeleteReturn = async (id: string) => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      toast.error(`Failed to delete return: ${errorMessage}`);
    } finally {
      setLoading(false);
    }
  };
              ...
              <div className="sm:w-2/3">
                ...
                <div className="relative rounded-md shadow-sm">
                  ...
                  <input
                    ...
                    placeholder="Search by return number, supplier or purchase..."
                    value={searchTerm}
                    onChange={(e) => {
                ...
                <tr>
                  ...
                  >
                    <div className="flex items-center">
                      Return #
                      <span className="ml-1">{getSortIcon('returnNumber')}</span>
                    </div>
                  </th>
                  <th 
                    scope="col" 
                  ...
                  >
                    <div className="flex items-center">
                      Refund
                      <span className="ml-1">{getSortIcon('refundStatus')}</span>
                    </div>
                  </th>
                  ...
                  returns.map((returnItem) => (
                    <tr key={returnItem._id} className="hover:bg-gray-50">
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-blue-600 hover:text-blue-800">
                        <Link href={`/purchase/return/${returnItem._id}`}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {returnItem.purchaseReference ? (
                          <Link href={`/purchase/${returnItem.purchase}`} className="text-blue-600 hover:text-blue-800">
                            {returnItem.purchaseReference}
                          </Link>
                        ) : (
                          <span>-</span>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {calculateTotalQuantity(returnItem)}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                          returnItem.refundStatus === 'Fully Refunded' ? 'bg-green-100 text-green-800' :
                          returnItem.refundStatus === 'Partially Refunded' ? 'bg-yellow-100 text-yellow-800' :
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <div className="flex justify-end space-x-3">
                          <Link href={`/purchase/return/${returnItem._id}`} className="text-gray-600 hover:text-gray-900">
                            <EyeIcon className="h-5 w-5" aria-hidden="true" />
                          </Link>
                          <Link href={`/purchase/return/edit/${returnItem._id}`} className="text-indigo-600 hover:text-indigo-900">
          ...
          {returns.length > 0 && (
            <div className="px-6 py-4 bg-gray-50 border-t border-gray-200 flex items-center justify-between">
              <div className="text-sm text-gray-500">
                Showing {(currentPage - 1) * itemsPerPage + 1} to {Math.min(currentPage * itemsPerPage, totalReturns)} of {totalReturns} returns
              </div>
