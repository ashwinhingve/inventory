...
export default function PaymentsPage() {
  ...
  const fetchPayments = useCallback(async () => {
    ...
    try {
      ...
      // Make API request
      const response = await fetch(`/api/sales/payments?${params.toString()}`);
  ...
  }, [setLoading, searchTerm, statusFilter, dateFilter, sortField, sortDirection, currentPage, itemsPerPage]);
  ...
  const handleDeletePayment = async (id: string) => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`/api/sales/payments/${id}`, {
        method: 'DELETE',
      });
              ...
              <div className="sm:w-2/3">
                ...
                <div className="relative rounded-md shadow-sm">
                  ...
                  <input
                    ...
                    placeholder="Search by reference number, customer or invoice..."
                    value={searchTerm}
                    onChange={(e) => {
                  ...
                  payments.map((payment) => (
                    <tr key={payment._id} className="hover:bg-gray-50">
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-blue-600 hover:text-blue-800">
                        <Link href={`/sales/payment-in/${payment._id}`}>
                          {payment.reference}
                        </Link>
                      </td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {payment.invoiceNumber ? (
                          <Link href={`/sales/invoice/${payment.invoice}`} className="text-blue-600 hover:text-blue-800">
                            {payment.invoiceNumber}
                          </Link>
                        ) : (
                          <span>-</span>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <div className="flex justify-end space-x-3">
                          <Link href={`/sales/payment-in/${payment._id}`} className="text-gray-600 hover:text-gray-900">
                            <EyeIcon className="h-5 w-5" aria-hidden="true" />
                          </Link>
                          <Link href={`/sales/payment-in/edit/${payment._id}`} className="text-indigo-600 hover:text-indigo-900">
